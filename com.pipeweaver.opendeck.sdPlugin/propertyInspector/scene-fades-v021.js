"use strict";
function installSceneFades021(){
 const inner=document.getElementById('baseFrame')?.contentWindow?.document?.getElementById('sceneFrame')?.contentWindow;
 if(!inner?.__features018){setTimeout(installSceneFades021,50);return}
 if(inner.__fades021)return;
 const script=inner.document.createElement('script');script.textContent=fadeChoices.toString()+';'+fadeSelection.toString()+';('+sceneFades021.toString()+')()';inner.document.head.append(script);script.remove();
}
window.addEventListener('load',installSceneFades021);
function sceneFades021(){
 window.__fades021=true;
 const oldSet=setType,oldFields=fields,oldSummary=summary,oldRender=render;
 setType=function(i,type){if(type!=='volumeFade')return oldSet(i,type);const old=operations[i]||{};operations[i]={type,kind:'source',mix:'A',volume:0,seconds:3,condition:old.condition||{type:'always'},onFailure:old.onFailure||'stop'};save();render()};
 window.setFadeKind=function(i,kind){const old=operations[i];operations[i]={type:'volumeFade',kind,mix:'A',volume:old.volume,seconds:old.seconds,condition:old.condition,onFailure:old.onFailure};save();render()};
 ...
 fields=function(op,i){
  ...
  return '<div class="field"><label>Fade control</label><select onchange="setFadeKind('+i+',this.value)">'+[['application','Application'],['input','Physical Input'],['output','Physical Output'],['source','Source'],['target','Target']].map(([v,n])=>'<option value="'+v+'" '+(v===op.kind?'selected':'')+'>'+n+'</option>').join('')+'</select></div>'+
   ...
   '<div class="field"><label>Fade to %</label><input type="number" min="0" max="100" value="'+esc(op.volume??0)+'" onchange="setField('+i+',\'volume\',this.value)"></div><div class="field"><label>Seconds</label><input type="number" min="0" max="120" step="0.1" value="'+esc(op.seconds??3)+'" onchange="setField('+i+',\'seconds\',this.value)"></div><div class="appnote">Waits for this fade to finish. Missing devices/applications or an interrupted fade follow the failure policy below. Use a condition to skip an application that is not running. Mute is unchanged.</div>'+oldFields(op,i);
 };
 summary=function(op){return op.type==='volumeFade'?'Volume Fade • '+op.kind+' • '+op.volume+'% • '+op.seconds+'s • '+oldSummary(op):oldSummary(op)};
