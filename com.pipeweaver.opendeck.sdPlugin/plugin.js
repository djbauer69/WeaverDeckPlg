...
/* PipeWeaver Control for OpenDeck v0.18.1
...
for(const method of ["log","error","warn"]){
  ...
  console[method]=(...args)=>original(...args.map(v=>typeof v==="string"?v.replace(/\[v0\.(?:11\.2|12\.2|15\.0|15\.1|16\.0)\]/g,"[v0.18.1]"):v));
}
...
class WeaverVisualWebSocket {
  constructor(...args){
    ...
    this._ws.onmessage=(ev)=>{
      try{visualLayer.handleIncoming(this,ev)}catch(e){console.error("[v0.18.1] application visuals inbound error:",e?.stack||e)}
      try{sceneVisualLayer.handleIncoming(this,ev)}catch(e){console.error("[v0.18.1] Scene visuals inbound error:",e?.stack||e)}
      try{if(sceneFileLayer.handleIncoming(this,ev))return}catch(e){console.error("[v0.18.1] Scene file I/O inbound error:",e?.stack||e)}
      try{if(sceneLibraryLayer.handleIncoming(this,ev))return}catch(e){console.error("[v0.18.1] Scene Library inbound error:",e?.stack||e)}
  ...
  send(data){
    ...
    try{data=sceneVisualLayer.handleOutgoing(this,data)}catch(e){console.error("[v0.18.1] Scene visuals outbound error:",e?.stack||e)}
...
console.error("[v0.18.1] artwork, resilient application identity, cached app discovery, Scene visuals, Scene Library, Source volume-link, and Smart Scenes enabled");
