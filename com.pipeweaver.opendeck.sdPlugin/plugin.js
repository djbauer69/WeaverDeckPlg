...
/* PipeWeaver Control for OpenDeck v0.14.0
 * v0.14.0 keeps the proven v0.11.2 control engine intact and adds an isolated
...
for(const method of ["log","error","warn"]){
  ...
  console[method]=(...args)=>original(...args.map(v=>typeof v==="string"?v.replace(/\[v0\.11\.2\]/g,"[v0.14.0]"):v));
}
...
class WeaverVisualWebSocket {
  constructor(...args){
    ...
    this._ws.onmessage=(ev)=>{
      try{visualLayer.handleIncoming(this,ev)}catch(e){console.error("[v0.14.0] application visuals inbound error:",e?.stack||e)}
...
console.error("[v0.14.0] application artwork layer enabled");
require("./plugin-core");
