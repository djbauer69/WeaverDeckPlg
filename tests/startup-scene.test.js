...
test('composed runner reports success, validation failure and continued failure accurately',async t=>{
 ...
 let s=fs.readFileSync(root+"/plugin-core.js","utf8");s=s.slice(0,s.indexOf('diag("startup",'));vm.runInContext(s,c);vm.runInContext('getStatus=async()=>({Status:{audio:{profile:{}}}})',c);
