...
WeaverDeck v0.23.0 (review build) provides **12 sidebar actions** and retains all **57 operations**. Drag Application, Physical Input, Physical Output, Route, Source or Target onto a key, then choose an operation from the **Action** dropdown in the inspector header. The tables below list these selectable operations. Audio Buffer Size, Audio Engine Restart, Default Device Set, Scene, Scene Startup and Status remain separate actions. On encoders, the five audio groups select their Volume Dial operation automatically.

Existing installations need the [offline profile migration](../UPGRADE-v0.23.0.md) before installing the compact manifest. The stable release is still v0.22.0 during review.
