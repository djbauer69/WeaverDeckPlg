...
It checks the final ZIP checksum, Git source bytes/modes and main-branch ancestry; prepares the five wiki pages while preserving other pages; uploads the ZIP to a draft release; downloads and checks the asset; pushes the wiki without force; then publishes **WeaverDeck v0.24.1** as latest and stable. The publisher uses the draft creation response directly, avoiding a second collection lookup that can miss a newly created draft. If interrupted, add `--release-id ID` using the printed GitHub release ID to resume that exact draft. Its tag and target commit are checked before any upload. Existing identical uploads can be reused. An unexpected existing asset/tag stops publication rather than replacing it. No other releases, tags, branches or history are removed. A network failure can leave a draft or an updated wiki; rerun the same command to complete it.
